...
from mini_cyborg import DefenderCyborgEnv
...
logdir = "logs/train_PPO_seed_" + str(seed) + "_" + time.strftime("%Y%m%d_%H%M%S")

# Instantiate the PPO model (MlpPolicy is suitable for vector observations)
model = PPO(
    "MlpPolicy", env, verbose=1, seed=seed, tensorboard_log=logdir, device="cpu"
